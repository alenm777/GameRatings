...
function JuegosPorGeneroId({gameList, selectedGeneroName})  {
useEffect(() => {

},[])
  return (
    <div>
        <h2 className='font-bold text-[30px] dark:text-white mt-5'>{selectedGeneroName} </h2>
    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3
    gap-6 mt-5'>
        {gameList.map((item)=> (
            <div className='bg-[#76a8f75e] p-3 rounded-lg pb-12 h-full
            hover:scale-110 translate-all ease-in-out duration-300 cursor-pointer'>
